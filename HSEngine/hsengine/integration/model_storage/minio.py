...
class MinIOModelStorage(BaseModelStorage):
    ...
    def __init__(self,
        ...
        :type sub_path: str, optional
        :param framework: model framework, defaults to ""
        :type framework: str, optional
        :param bucket: bucket to store the model, defaults to "models"
        ...
        """
        super(MinIOModelStorage, self).__init__(framework)
        self.bucket = bucket
        ...
        logger.debug("Initialized MinIOModelStorage with endpoint: {}, "
    ...
    def save(self, model_object, dest=""):
        ...
        # just upload it. Otherwise we try the common frameworks
        # to save the model.
        logger.debug("Upload model object: {} of type: {}, for framework: {}"
                     .format(model_object, type(model_object), self.framework))
        if isinstance(model_object, str):
            ...
            if not dest:
                raise ValueError("dest cannot be empty when saving in-memory model object")
            import tempfile
            local_path = os.path.join(tempfile.mkdtemp(), dest)
            super(MinIOModelStorage, self).save(model_object, local_path)
            return self.save(local_path, dest)
    ...
    def upload_objects(self, objects):
        ...
        for obj in objects:
            ...
            file = obj['obj']
            logger.debug("Uploading {} to {}".format(file, dest))
        ...
        logger.info("Uploaded model objects into path: {}".format(model_path))
        return model_path
